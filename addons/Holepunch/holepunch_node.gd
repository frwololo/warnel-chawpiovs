...
signal error(error_text, error_id)
	...
	if not recieved_peer_confirm and greets_sent < response_window:
		for p in peer.keys():
			...
			if greets_sent == response_window:
				...
				emit_signal("error", "Receiving no confirm. Starting port cascade", 1)
...
func start_peer_contact():	
	...
	if err != OK:
		emit_signal("error", "Error listening on port: " + str(own_port) +" Error: " + str(err), err)
...
func start_traversal(id, is_player_host, player_name):
	...
	if err != OK:
		emit_signal("error", "Error listening on port: " + str(rendevouz_port) + " to server: " + rendevouz_address, err)
